...
from jaxformer.jaxformer.utils import print_time
from jaxformer.jaxformer.data.iterator_resumable import list_files, create_resumable_iter, create_mock_iter
...
DEEPSPEED_CONFIG = \
{
    ...
    'optimizer': {'type': 'AdamW', 'params': {'lr': 2e-05, 'betas': [0.9, 0.999], 'eps': 1e-08, 'weight_decay': 0.0}},
    'scheduler': {'type': 'WarmupLR', 'params': {'warmup_min_lr': 0, 'warmup_max_lr': 2e-05, 'warmup_num_steps': 100}},
    'zero_optimization': {
        'stage': 3,
...
def create_args(args=argparse.Namespace()):
    ...
    # args.model = '/data/kiho/autocode/codegen/codegen6B_mono_cache'
    args.model = "Salesforce/codegen-6B-multi"
    args.cache_dir = '/data/kiho/autocode/codegen/codegen6B_multi_cache'

    args.deepspeed_config = DEEPSPEED_CONFIG
    
    # jaxformer's preprocessing (snitizing methods are needed)
    # https://github.com/salesforce/jaxformer/tree/main/preprocess
    
    args.data_train_set = '/data/kiho/autocode/dataset/code_segments_py150k.json' # tuning_dataset_with_csv_format
...
def train(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model, config=config, cache_dir=args.cache_dir)
    ...
    for step in range(args.opt_steps_train+1):
        ...
        if args.debug_fixed_batch:
            ...
            # (data, data_file, data_batch) = loader_train_iter
