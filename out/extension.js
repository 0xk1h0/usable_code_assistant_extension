...
async function activate(context) {
    ...
    const textDocumentProvider = new class {
        async provideTextDocumentContent(uri) {
            ...
            try {
                ...
                const result = await axios_1.default.get(`https://seclab.co.kr/autocompleteNLtoCode`, {
                    httpsAgent: agent,
                    params: { input: word },
                    // 	headers: {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('CodeGen.chooseOption', (fn) => {
        if (!selectedEditor) {
            return;
        }
        try {
